...
│
├── agent/
│   ├── react_agent.py              # 传统模式 Agent（Function Calling）
│   ├── mcp_react_agent.py          # MCP 模式 Agent（核心）
│   ├── mcp_client.py               # MCP 客户端连接工具
│   ├── mcp_middleware.py           # MCP 模式中间件管理器
│   └── tools/
│       ├── agent_tools.py          # 传统模式工具定义（@tool 装饰器）
│       └── middleware.py           # 传统模式中间件（@wrap_tool_call 等）
│
├── mcp_server/
│   ├── __init__.py
...
- **MCP Server**（`mcp_server/server.py`）：使用 `FastMCP` 框架，以 `@mcp_server.tool()` 装饰器定义 6 个工具，通过 **stdio** 传输运行。
- **MCP Client**（`agent/mcp_react_agent.py`）：使用 `MultiServerMCPClient` 连接 Server，将 MCP 工具转换为 LangChain 工具供 Agent 使用。
...
| 工具 | 描述 | MCP Server | 传统模式 |
|------|------|:----------:|:--------:|
| `rag_summarize` | 从向量知识库检索参考资料 | `mcp_server/server.py` | `agent/tools/agent_tools.py` |
| `get_weather` | 提取城市并查询实时天气 | `mcp_server/server.py` | `agent/tools/agent_tools.py` |
| `get_user_id` | 获取当前用户 ID | `mcp_server/server.py` | `agent/tools/agent_tools.py` |
| `get_current_month` | 获取当前月份 | `mcp_server/server.py` | `agent/tools/agent_tools.py` |
| `fetch_external_data` | 获取指定用户指定月份的使用记录 | `mcp_server/server.py` | `agent/tools/agent_tools.py` |
| `fill_context_for_report` | 触发报告模式信号 | `mcp_server/server.py` | `agent/tools/agent_tools.py` |

---

## 中间件机制

### MCP 模式中间件（`agent/mcp_middleware.py`）
...
### 传统模式中间件（`agent/tools/middleware.py`）
...
扩展知识库：将新的 `.txt` 或 `.pdf` 文件放入 `data/`，重启服务后自动加载。
